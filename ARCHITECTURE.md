...
Self-hosted панель управления VPN на AmneziaWG и WireGuard: один Node-процесс,
который поднимает туннель (движок выбирается `WG_ENGINE`, по умолчанию для новых
установок — AmneziaWG), управляет пирами (юзерами), собирает статистику трафика
в SQLite и отдаёт красивую веб-морду (React SPA, русский язык).
